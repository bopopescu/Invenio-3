...
function PageViewArea(container, eventsBus){
	...
	slide: prepareBoundHandler(function(event, ui){
	    this.lblZoomScale.html(ui.value + "%");
    ...
    // degree character : " + String.fromCharCode(176) + "
//    this.lblCont = $("<div></div>");
    this.lblRotationAngle = $("<div style=\"position: relative; height:50px; display: block;\"></div>");

    this.dispRotationAngle = new FractionDisplay(this.lblRotationAngle, String.fromCharCode(960));
	...
	value: 0,
	slide: prepareBoundHandler(function(event, ui){
//	    this.lblRotationAngle.html(ui.value + String.fromCharCode(176));
	    var angle = ui.value * 3;
	    //	    this.lblRotationAngle.html((angle / 180) + String.fromCharCode(960));
	    this.dispRotationAngle.setNumber(angle, 180);
    ...
    this.eventsBus.registerHandler(Presenter.setPage, this.id,
				   function(subjectId, page){
				       ...
				       this.clearMarkup();
				       for (var fId in page.figures){
					   this.addFigureRectangles(page.figures[fId]);
				       }
				   }, this);
    ...
    this.eventsBus.registerHandler(Presenter.figureCreated, this.id,
				  function(subjectId, figure){
				      if (figure.page == this.page){
					  this.addFigureRectangles(figure);
				      }
				  }, this);
	...
	function(subjectId, figure){
		...
		if (this.figToRectangles[figure.id] != undefined){
		    this.figToRectangles[figure.id].setRectangle(figure.boundary);
...
//    this.imgElement.rotate(newAngle);
...
PageViewArea.prototype.addFigureRectangles = function(figure){
    var rectangle = this.addRectangle(figure.boundary);
...
PageViewArea.prototype.setRotationAngle = function(newAngle){
...
PageViewArea.prototype.adjustImage = function(){
    var css = getTransformedCSS(this.rotationAngle, this.scale,
...
PageViewArea.prototype.selectRectangle = function(onFinish){
	...
	onSelectEnd: function(img, selection){
	    me.imgElement.imgAreaSelect({ remove:true });
...
PageViewArea.prototype.setPageImage = function(image){
    if (image != null && image != undefined){
	this.imgElement.attr("src", image.url);
	this.adjustImage();
    }
};

PageViewArea.prototype.hideAdditionalInterface = function(){
    this.controlsArea.css({display: "none"});
    this.rectanglesContainer.css({display: "none"});
};


PageViewArea.prototype.showAdditionalInterface = function(){
    this.controlsArea.css({display: "block"});
    this.rectanglesContainer.css({display: "block"});
};




/** an interface elelemnt displaying a rectangle in the page view interface */
...
PageViewRectangle.prototype.refresh = function(){
    ...
    // now the rotation corresponding to real angle
    var rotateToOriginCss = "rotate(" + (-this.rectangle.angle) + "deg)";

    var transCss = returnFromOrthoCss + " " + rotateCss + " " + bringToOrthoCss;


    this.rootUIElement.css({
	position: "absolute",
	width: effWidth + "px",
	height: effHeight + "px",
	left: effX + "px",
